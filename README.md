...
`Alloc`: If a non-null pointer is returned, it shall be valid until `Free` is called for the pointer. It shall allocate at least the amount of bytes requested and the pointer shall be aligned to `max_align_t`.
